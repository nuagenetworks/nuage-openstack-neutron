...
class TestNuageRestproxy(testtools.TestCase):
    ...
    def test_post_on_res_exists(self, log_mock, rest_call_mock):
        ...
        log_mock.debug.assert_any_call('Received 409 from VSD with '
                                       'internalErrorCode 2510. Trying '
                                       'on_res_exists to recover.')
        log_mock.debug.assert_called_with('Recovery from 409 successful.')
    ...
    def test_post_on_res_exists_neg(self, log_mock, rest_call_mock):
        ...
        log_mock.debug.assert_any_call('Received 409 from VSD with '
                                       'internalErrorCode 2510. Trying '
                                       'on_res_exists to recover.')
        log_mock.debug.assert_called_with('Recovery from 409 unsuccessful.')
