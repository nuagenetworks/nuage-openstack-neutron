...
class NuageFWaaSPlugin(base_plugin.BaseNuagePlugin,
    ...
    The 'fw-lock' only locks within a single worker (intentional). The locks
    avoid lock wait timeout issues which can occur when a single worker
    processes 2 requests of the same resource. It's needed in kilo only. In
    Liberty the pymysql driver should be used and fixes these issues.
    """
    def __init__(self):
    ...
    def update_firewall_rule(self, context, id, firewall_rule):
        with lockutils.lock('fw_lock'),\
                self.db_lock_by_rule(context, id):
    ...
    def update_firewall_policy(self, context, id, firewall_policy):
        with lockutils.lock('fw_lock'),\
                self.db_lock_by_policy(context, id):
    ...
    def update_firewall(self, context, id, firewall):
        with lockutils.lock('fw_lock'),\
                self.db_lock_by_firewall(context, id):
            if firewall['firewall'].get('firewall_policy_id'):
                self._lock_policy(context.session,
                                  firewall['firewall']['firewall_policy_id'])
