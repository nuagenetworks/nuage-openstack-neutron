...
class SwitchportNotFound(nexc.NotFound):
    message = _("Switchport %(id)s does not exist")


class SwitchportParamDuplicate(nexc.InUse):
...
class SwitchportInUse(nexc.InUse):
...
class SwitchportBindingNotFound(nexc.NotFound):
    message = _("Switchport binding %(id)s does not exist")


class SwitchTypeNotSupported(nexc.Conflict):
