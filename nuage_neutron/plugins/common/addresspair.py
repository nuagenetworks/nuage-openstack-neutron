...
class NuageAddressPair(BaseNuagePlugin):
    ...
    def _create_vips(self, context, nuage_subnet_id, port, nuage_vport):
        nuage_vip_dict = dict()
        enable_spoofing = False
        vsd_subnet = self.nuageclient.get_subnet_or_domain_subnet_by_id(
            nuage_subnet_id, required=True)
    ...
    def _update_vips(self, context, nuage_subnet_id, port, nuage_vport,
                     deleted_addr_pairs):
        ...
        if vips_add_list:
            ...
            self._create_vips(context, nuage_subnet_id, port_dict, nuage_vport)
    ...
    def _process_allowed_address_pairs(self, context, port, vport,
        ...
        if subnet_mapping:
            if vport:
                if create:
                    self._create_vips(context,
                                      subnet_mapping['nuage_subnet_id'],
                                      port,
                                      vport)
                else:
                    self._update_vips(context,
                                      subnet_mapping['nuage_subnet_id'],
                                      port,
                                      vport,
                                      delete_addr_pairs)
