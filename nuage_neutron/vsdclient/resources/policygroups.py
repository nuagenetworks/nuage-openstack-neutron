    ...
    def create_nuage_sec_grp_for_port_sec(self, params):
        ...
        try:
            ...
            if (e.code == restproxy.REST_CONFLICT and
                ...
                policygroup = self.get_sg_policygroup_by_external_id(
                    sg_id=params_sg['sg_id'], sg_type=params_sg['sg_type'],
                    required=True)
                return policygroup[0]['ID']
            else:
                raise
        try:
            self.create_nuage_sec_grp_rule_for_port_sec(params)
        except Exception:
            with excutils.save_and_reraise_exception():
                helper.process_rollback(self.restproxy, to_rollback)
        return params['sg_id']
