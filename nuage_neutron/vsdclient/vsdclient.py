...
from nuagenetlib.common import cms_id_helper
from nuagenetlib.common import constants
from nuagenetlib.common import gw_helper
...
from nuagenetlib.resources import domain
from nuagenetlib.resources import fwaas
from nuagenetlib.resources import gateway
from nuagenetlib.resources import l2domain
from nuagenetlib.resources import netpartition
from nuagenetlib.resources import policygroups
from nuagenetlib.resources import vm
from nuagenetlib.restproxy import ResourceNotFoundException as rp_nfe
from nuagenetlib.restproxy import RESTProxyError as rp_e
from nuagenetlib.restproxy import RESTProxyServer

LOG = logging.getLogger(__name__)

ResourceNotFoundException = rp_nfe
RESTProxyError = rp_e
...
class VsdClient(object):
    ...
    # leave as separate method for easy mocking
    def initialize(self):
        server = cfg.CONF.RESTPROXY.server
        serverauth = cfg.CONF.RESTPROXY.serverauth
        serverssl = cfg.CONF.RESTPROXY.serverssl
        server_timeout = cfg.CONF.RESTPROXY.server_timeout
        server_max_retries = cfg.CONF.RESTPROXY.server_max_retries
        ...
        self.restproxy = RESTProxyServer(server,
                                         base_uri,
                                         serverssl,
                                         serverauth,
                                         auth_resource,
    ...
    def create_cms(self, name):
        ...
        if not cms.validate(response):
            ...
            raise RESTProxyError(cms.error_msg)
    ...
    def get_usergroup(self, tenant, net_partition_id):
        return helper.get_usergroup(self.restproxy,
    ...
    def delete_user(self, id):
        ...
        if not nuageuser.delete_validate(response):
            ...
            raise RESTProxyError(nuageuser.error_msg)
    ...
    def delete_group(self, id):
        ...
        if not nuagegroup.delete_validate(response):
            ...
            raise RESTProxyError(nuagegroup.error_msg)
    ...
    def get_router_np_id(self, router_id):
        ...
        if not nuage_l3_domain.validate(response):
            raise RESTProxyError(nuage_l3_domain.error_msg)
    ...
    def get_gateway_vports(self, tenant_id, netpart_id, filters):
        return self.nuagegw.get_gateway_vports(tenant_id,
                                               netpart_id,
                                               filters)
    ...
    def associate_fip_to_vips(self, params):
        self.vm.associate_fip_to_vips(params)
